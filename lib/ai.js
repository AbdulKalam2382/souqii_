  ...
  try {
    const completion = await groq.chat.completions.create({
      messages: [
        {
          role: "system",
          content: "You are an AI search assistant for a PC parts store. Based on the query, return a JSON array of product IDs from the catalog that best match. Return ONLY the array. STRICT RULE: If the query is completely irrelevant to PC parts, gaming, electronics, or the provided catalog, you MUST return an empty array []."
        },
        {
          role: "user",
          content: `Query: "${query}"
          Catalog: ${JSON.stringify(products.map(p => ({ id: p.id, name: p.name, price: p.price, specs: p.specs })))}`
        }
      ],
      model: "llama-3.1-8b-instant"
    });

    // Handle potential non-JSON response if model goes rogue, but Llama is usually good
    ...
    // Bulletproof Fallback: Local Keyword matching
