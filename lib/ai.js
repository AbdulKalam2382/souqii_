  ...
  try {
    const completion = await groq.chat.completions.create({
      messages: [
        {
          role: "system",
          content: "You are an AI search assistant for a PC parts store. Based on the query, return a JSON array of product IDs from the catalog that best match. Return ONLY the array."
        },
        {
          role: "user",
          content: `Query: "${query}"
