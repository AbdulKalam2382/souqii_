...
1. CATEGORY: If the user asks for a specific category (e.g., "RAM", "GPU", "CPU"), ONLY return product IDs that belong to that category. Trust the catalog's category slugs/names for this.
2. BUDGET: If the user specifies a budget (e.g., "under 100 KD"), strictly include ONLY products where price <= budget.
3. ADVICE: Always provide expert, friendly advice. If you find no items, explain WHY (e.g., "Sorry, I don't have GPUs under 10 KD currently").
   - IF NO MATCHES exist, set 'productIds' to [] and tell the user you'll notify them once new stock arrives.
4. HONESTY: If you recommend an item slightly over budget because it's a great deal, mention that clearly. Otherwise, stick to constraints.
...
${JSON.stringify(catalog, null, 2)}`
    ...
    // We trust the AI's decision here as the "Supreme Expert" to ensure a conversational and accurate outcome.
    const matchedProducts = (response.productIds || [])
