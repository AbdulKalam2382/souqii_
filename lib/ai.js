...
1. CATEGORY: If the user asks for a specific category (e.g., "RAM", "GPU", "CPU"), ONLY return product IDs that belong to that category. Do not suggest miscellaneous items.
2. BUDGET: If the user specifies a budget (e.g., "under 100 KD"), ONLY return product IDs where price <= budget.
3. RELEVANCE: Prioritize the best matches first.

RESPONSE FORMAT (JSON):
{
  "message": "Friendly, expert advice. Briefly explain why these parts fit the user's criteria.",
  "productIds": [array of matching product IDs]
}

If no products match perfectly, explain why and suggest the closest alternatives within the catalog.`
        },
        {
          role: "user",
...
${JSON.stringify(catalog, null, 2)}`
    ...
    // Map product IDs to full product objects AND apply strict local filtering
    let matchedProducts = (response.productIds || [])
      .map(id => products.find(p => p.id === parseInt(id)))
      .filter(Boolean);

    // Apply hard budget filter if detected
    if (maxPrice) {
      matchedProducts = matchedProducts.filter(p => p.price <= maxPrice);
    }

    // Apply category keyword heuristic if the AI is loose
    const lowerQuery = query.toLowerCase();
    if (lowerQuery.includes('ram')) {
        matchedProducts = matchedProducts.filter(p => (p.name + " " + (p.categories?.name || "")).toLowerCase().includes('ram'));
    } else if (lowerQuery.includes('gpu') || lowerQuery.includes('graphics')) {
        matchedProducts = matchedProducts.filter(p => (p.name + " " + (p.categories?.name || "")).toLowerCase().includes('gpu') || (p.categories?.slug === 'gpu'));
    }

    let finalMessage = response.message || "Here's what I found for you!";

    // If our strict local filters stripped everything, override the message to be honest
    if (matchedProducts.length === 0) {
      finalMessage = "No relevant match found in our current inventory. We will notify you once new stock arrives that fits your criteria! 📢";
    }
